    ...
    try {
      ...
      if (d.ok && d.result) {
        ...
        if (d.provider === 'openai') {
          setApiNote('Analysis generated by AI. Verify details before outreach.')
        }
      } else {
        const reason = d.reason ?? 'Analysis failed. Please try again.'
        setApiNote(reason)
        setProspects(all => all.map(x => (x.id === p.id ? { ...x, stage: 'Analyzed' } : x)))
        setSelected({ ...p, stage: 'Analyzed' })
      }
    } catch {
      setApiNote('Analysis failed. Check your connection and try again.')
      setProspects(all => all.map(x => (x.id === p.id ? { ...x, stage: 'Analyzed' } : x)))
      setSelected({ ...p, stage: 'Analyzed' })
    } finally {
